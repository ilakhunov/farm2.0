...
import '../../view_history/data/view_history_repository.dart';
...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  void initState() {
    ...
    if (widget.product != null) {
      ...
      // Record view and load data for pre-loaded product
      if (_product != null) {
        _viewHistoryRepository.recordView(_product!.id);
        _loadFavoriteStatus(_product!.id);
        _loadRatingStats(_product!.id);
      }
  ...
  Future<void> _loadProduct() async {
    ...
    try {
      ...
      // Record view history
      _viewHistoryRepository.recordView(product.id);
      
      // Load favorite status
      _loadFavoriteStatus(product.id);
      
      // Load rating stats
      _loadRatingStats(product.id);
  ...
  Future<void> _loadFavoriteStatus(String productId) async {
    try {
      final isFavorite = await _favoriteRepository.isFavorite(productId);
      if (mounted) {
        setState(() {
  ...
  Future<void> _loadRatingStats(String productId) async {
    try {
      final stats = await _ratingRepository.getProductRatingStats(productId);
      if (mounted) {
        setState(() {
          _ratingStats = stats;
        });
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      if (_isFavorite) {
        await _favoriteRepository.removeFromFavorites(_product!.id);
        if (mounted) {
          setState(() {
            _isFavorite = false;
        ...
        await _favoriteRepository.addToFavorites(_product!.id);
        if (mounted) {
          setState(() {
            _isFavorite = true;
            _isLoadingFavorite = false;
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Изображение товара (поддержка множественных изображений)
